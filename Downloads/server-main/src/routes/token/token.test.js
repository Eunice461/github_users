...
describe('TOKEN API', () => {
    ...
    afterAll(async () => {
      ...
      test('Should get single token by id', async () => {
        const res = await request(app)
        .get(`/v1/token/624af651f00b88fbb7880c0e`)
        .set('Authorization', 'Bearer ' + token)
          .send()
          .expect('Content-Type', /json/)
          .expect(200)
      ...
      test('Should retrieve all tokens created by a user', async ()  => {
        const res = await request(app)
          .get(`/v1/token/`)
          .set('Authorization', 'Bearer ' + token)
          .send()
       ...
       */
       ...
       test('Should delete token', async () => {
        const res = await request(app)
        .delete(`/v1/token/624da3f0483fe90cca454245`)
        .set('Authorization', 'Bearer ' + token)
          .send()
          .expect('Content-Type', /json/)
          .expect(200)
