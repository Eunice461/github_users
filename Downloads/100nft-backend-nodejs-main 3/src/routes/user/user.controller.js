...
async function httpGetSingleUser(req, res) {
  try {
    ...
    .select('-password')
    .populate({
      path: 'originalTokenForUser', 
      select: "nftName DesignUrl tokenId likes originalTokenId description isPropose",
      populate: [{
        path: "auctions"
      }]
    })
    .populate({
      path: 'proposes', 
      select: "nftName DesignUrl tokenId likes description",
      populate: [{
        path: "auctions"
      }]
    })
    .populate({
      path: "auctions",
      populate: [{
        path: "allBids tokenId originalId", 
        select: ' _id price user nftName DesignUrl description tokenId ',
      }],
...
async function httpGetAllUserAuctionBid(req, res){
  try {
    ...
    const bid = await Bid.find({_id: getAuction}).populate({
      ...
      populate: {
        path: 'tokenId originalId highestBider',
...
async function httpGetNewlyRegisterUserPerWeek(req, res){
  try {
    const user = await User.findById(req.user.userId);
    if(!user){
        return res.status(StatusCodes.BAD_REQUEST).json("No user found")
