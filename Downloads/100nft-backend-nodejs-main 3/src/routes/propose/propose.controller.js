...
async function httpCreateProposeForReplica(details, user, resultIdd, req, res){
    try {
      ...
      if(getOriginalInfo){
        ...
        try {
            const word = details.nftName
            const tokenId = resultIdd
...
const createdPropose = await newPropose.save();
    ...
    if(getOriginalInfo){
      ...
      if(!createdPropose){
       ...
       //res.status(StatusCodes.OK).json(createdPropose)
...
async function httpGetAllToken(req, res){
    ...
    try {
      const token = await Propose.find({}).select('-amountGenerated')
      .populate('colors')
      .sort({createdAt: -1})
      .skip(skip)
      .limit(limit)
  ...
  async function httpCreateCommisssion( req, res){
    try{
      ...
      if(expiryDate){
        if(!dayJsDate(expiryDate, "YYYY-MM-DD", true).isValid()){
